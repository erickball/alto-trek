...
function loadSystemWithDisk() {
    ...
    bootButton.disabled = true;
    system.loadPack("images/" + diskName,
        function() {bootButton.disabled = false;});
}

window.onload = function() {
    // loadSystemWithDisk() re-enables bootButton in its completion callback,
    // once the disk image has actually downloaded. Do NOT enable it here:
    // booting before the pack is loaded starts the Alto with an empty drive
    // (white screen forever). Only matters when the disk loads slowly (CDN);
    // on localhost the race was invisible.
    loadSystemWithDisk();
