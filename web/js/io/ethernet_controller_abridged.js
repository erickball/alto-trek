...
var ethernetController = {
    ...
    joinWebSocketNetwork: function(url, connectionStateChangedCallback) {
        ...
        var self = this;
        this.network = new AltoWSNetwork(url,
            function gotNetworkPacket(frame) {
                // Drop frames claiming to come from our own address —
                // either a relay echo or an address collision.
                if (frame.length >= 2 && frame[1] === self.address) {
                    return;
                }
                self.incomingFrameQueue.push(frame);
                self.beginReadingFrame();
            },
